...
export default function GoogleOneTap() {
  ...
  const initializeGoogleOneTap = async () => {
    ...
    google.accounts.id.initialize({
      ...
      // Hint Google to show only NITR accounts
      hd: "nitrkl.ac.in",
      ...
      callback: async (response: CredentialResponse) => {
        try {
          ...
          // Enforce NITR email domain
          if (!user?.email?.endsWith("@nitrkl.ac.in")) {
            await supabase.auth.signOut();

            toast.error("Only NIT Rourkela accounts are allowed.");
            return;
          }
          ...
          toast.error(
            err instanceof Error ? err.message : "Google One Tap login failed.",
          );
        }
      },
    });
